...
try {
  ...
  if(apikeyInput !== `${key}`) return res.sendFile(invalidKey)
     ...
     var hasil = await getBuffer(`https://api.lolhuman.xyz/api/random/estetic?apikey=sayahafiz`)
       await fs.writeFileSync(__path + '/tmp/estetik.jpg', hasil)

         res.sendFile(__path + '/tmp/estetik.jpg')
} catch (e) {
     console.log(e)
	res.sendFile(error)
   }
